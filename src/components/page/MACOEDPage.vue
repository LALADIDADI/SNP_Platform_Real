          ...
          <el-col :span="9">
            <el-collapse v-model="activeName" accordion>
              <el-collapse-item title="算法简介 Algorithm introduction" name="1">
                <div>MACOED算法通过多目标启发式优化的方法，来计算和检测全基因组多位点上位性相互作用。</div>
              </el-collapse-item>
              <el-collapse-item title="参数描述 Parameter Description" name="3">
                <div><strong>maxIter:</strong> number of iteration(default = 50)</div>
                <div><strong>numAnt:</strong> number of ants(default = 100)</div>
                <div><strong>dimEpi:</strong> dimension of epistatic iteraction(default = 2)</div>
                <div><strong>alpha:</strong> p value threshold after Bonferroni correction(default = 0.1)</div>
                <div><strong>lambda:</strong> weight coefficient to non-dominated solutions(default = 2)</div>
                <div><strong>threshold:</strong> determing convergence speed and solutions diversity(default = 0.8)</div>
                <div><strong>tau:</strong> initial pheromone level for each locus(default = 1)</div>
                <div><strong>rou:</strong> evaporation rate in Ant Colony Optimizaion(default = 0.9)</div>
              </el-collapse-item>
...
import {MACOEDParamsUpload, MACOEDPollResultData, MACOEDResultShow, MACOEDForceStop} from '../../api/index'
...
export default {
  ...
  methods: {
    // 强制终止方法
    forceStop () {
      MACOEDForceStop().then(res => {
        console.log('MACOED进程已全部停止')
        // 提示
        this.$message({
          message: `MACOED进程已全部停止`,
          type: `success`
        })
      })
    },
    ...
    batchRun () {
      console.log('开始批处理')
      MACOEDBatchRequest(this.params).then(res => {
        console.log('MACOED批处理页面显示参数提交')
        console.log(res)
      })
